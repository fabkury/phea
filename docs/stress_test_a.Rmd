...
## TL;DR
These are these vignette's takeaways:
...
We do not want to put a `NULL` into that sum, because it would cause the entire result to permanently collapse to `NULL`. This is because any number plus `NULL` equals to `NULL`. To circumvent that, we will use a `coalesce()`. If a component C<sub>x</sub> is `NULL`, the number `0` will be used instead. So the actual formula we'll use is:  

> **P = coalesce(C<sub>1</sub><sup>value_as_number</sup>, 0) + coalesce(C<sub>2</sub><sup>value_as_number</sup>, 0) + coalesce(C<sub>2</sub><sup>value_as_number</sup>, 0) + ... + coalesce(C<sub>n</sub><sup>value_as_number</sup>, 0)**  
...
In each run, we use library `tictoc` to measure time the time spent.    
...
Here are the meanings of the columns:  
 ...
 - **re. rows**: Total number of rows in the phenotype returned by `calculate_formula()`.  
