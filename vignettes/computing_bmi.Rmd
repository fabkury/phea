...
The result `bmi` is a lazy table. Let us manipulate it further to produce a few aggregates:  
```{r, aggregates}
...
To see the SQL query underlying the phenotype, use dbplyr::sql_render(), or the `.clip_sql` option in `calculate_formula()`:  
```{r, sql_query}
